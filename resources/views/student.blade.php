...
<div class="flex h-screen">
    <main class="flex-1 overflow-y-auto p-10">
        <div class="flex justify-between items-center mb-8">
            <h1 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">Student Management</h1>
            <div class="flex space-x-2">
                <a href="{{ route('students.export') }}" class="px-4 py-2 bg-emerald-500 text-white rounded-md text-sm font-medium hover:bg-emerald-600 transition">
                    Export
                </a>
            ...
            <table id="studentsTable" class="w-full text-left border-collapse">
                <thead class="text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="pb-4 px-2">Programme</th>
                    </tr>
                </thead>
...
<div id="studentModal" class="fixed inset-0 z-50 hidden bg-slate-950/50 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl shadow-2xl p-8 border border-slate-200 dark:border-slate-800">
        <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100 mb-6">New Student</h2>
        ...
        <form id="addStudentForm" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <x-forms.label for="programme" required>Programme</x-forms.label>
                    <x-forms.select name="programme_id" id="prog_select" required disabled>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#dept_select').on('change', function() {
                ...
                if(deptId) {
                    $.get(`/get-programmes/${deptId}`, function(data) {
                        progSelect.empty().append('<option value="">Select Programme</option>').prop('disabled', false);
                        data.forEach(p => {
                            progSelect.append(`<option value="${p.id}">${p.name}</option>`);
                        });
                    });
                } else {
                    progSelect.prop('disabled', true);
                }
            });
        ...
        function toggleModal() {
            $('#studentModal').toggleClass('hidden');
        }
    </script>
@endsection